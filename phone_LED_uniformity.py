...
class Camera(object):
    ...
    def demosaic(self, image):
        ...
        if self.bayer == 'RGGB':
            red = quad1
            red = sd.convolve2d(red, non_green_kernel, mode='same')*255/1023.
            green = quad2+quad3
            green = sd.convolve2d(green, green_kernel, mode='same')*255/1023.
            blue = quad4
            blue = sd.convolve2d(blue, non_green_kernel, mode='same')*255/1023.
...
if __name__ == '__main__':
    if len(sys.argv):
        phone = Camera('/Users/fanchang/Downloads/20170403_4032x3032_YX_1_unpacked.raw',
                       dimension=(3032, 4032), bayer='RGGB')
    else:
        phone = Camera(sys.argv[1],
                       dimension=(3032, 4032), bayer='RGGB')
    # start = time.clock()
    image1 = phone.raw_reader
    output = phone.demosaic(image1)
    result = phone.LED_unifority(output, [50, 100])
    print(result[:, 4:7])