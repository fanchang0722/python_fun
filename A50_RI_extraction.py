...
folder = r'//Users/fanchang/Google Drive/A50/A50_EVT/Asub/light/20170418/image/Results'
files = os.listdir(folder)
Blemkey = 'blemishResults'
subkeys = ['resTable_Y_Luminance']
...
fd = open(os.path.join(os.getcwd(), 'Asub_EVT_RI_' + folder.split('/')[9] + '.csv'), 'w')
...
for filename in files:
	if filename.endswith('json') and filename.startswith('LOE6500'):
		...
		for subkey in subkeys:
			temp = np.hstack((temp, np.array(result[Blemkey][subkey])[0:5]))
		...
		fd.write('\n')
fd.close()
print(os.path.join(os.getcwd(), 'Asub_EVT_RI_' + folder.split('/')[9] + '.csv'))
